import { Board } from "../Helper_Files/board.ts";
import { tileDim } from "../Helper_Files/commonLib.ts";
import { game } from "../main.ts";
import { HEIGHT_UNUSED_FOR_TILES, my, AUTO_SAVE_SLOT_NAME, UNDO_APPEND, MAX_SUN, MAX_WATER, MIN_SUN, WATER_COEFFICIENT, randIntRange, cellDistManhattan, cellDistOctal, REDO_APPEND} from "../Helper_Files/commonLib.ts";
...
export class Farm extends Phaser.Scene {
    ...
    private SAVESIZE : number = 0;
    private history : string[] = [];
    private redoStack : string[] = [];
    private currentSeed : number = 0;
    private gameFrozen : boolean = false;
    private playerSprite : Player = new Player(this, 0, 0, "player", undefined);
    private cropSprites : Map<string, Crop> = new Map<string, Crop>();
    ...
    init() {
        ...
        const brambleberry = new plantType(7, 25, 15, my.crops.get("plantA")!);
        const wheat = new plantType(10, 20, 10, my.crops.get("plantB")!);
        const gilderberry = new plantType(4, 40, 25, my.crops.get("plantC")!); 
        ...
        this.plantTypes = [brambleberry, wheat, gilderberry];
    ...
    create() {
        ...
        this.createPlayer();
        if(typeof game.config.height === 'number') {
            const heldseed = this.add.text(0, game.config.height - HEIGHT_UNUSED_FOR_TILES, 'Held Seed: Plant ' + (this.currentSeed + 1), {fontSize: '20px', color:"#FEE"}).setOrigin(0);
            this.eventEmitter.addListener("hudUpdate", () => {heldseed.text = 'Held Seed: Plant ' + (this.currentSeed + 1)})
            const harvested = this.add.text(0, game.config.height - HEIGHT_UNUSED_FOR_TILES/2, 'Harvested Total: 0, 0, 0 ', {fontSize: '20px', color:"#EFE"}).setOrigin(0);
            this.eventEmitter.addListener("hudUpdate", () => {harvested.text = "Harvested Total: " + this.board.getPlant(0) + ", " + this.board.getPlant(1) + ", " + this.board.getPlant(2)})
        }
    ...
    createPlayer() {
        ...
        const playerScale = (this.tileWidth - 3) / (this.playerSprite.height * 2);
    ...
    drawBoard() {
        for (let i = 0; i < this.board.width; i++) {
            for (let j = 0; j < this.board.height; j++) {
                const curEntry = this.board.getEntry({x: i, y: j});
                const newRect = this.add.rectangle(i*this.tileWidth, j*this.tileHeight, this.tileWidth, this.tileHeight, 0x118c13, 1);
                newRect.setOrigin(0)
                ...
                const sunText = this.add.text(i*this.tileWidth, j*this.tileHeight, "☀️: " + curEntry.sunlight, fontSettings).setOrigin(0);
                const waterText = this.add.text(i*this.tileWidth + 2, j*this.tileHeight + fontSize + padding, "💧: " + curEntry.moisture, fontSettings).setOrigin(0);
                this.eventEmitter.on("updateCell" + i + j, () => {
                    const entry = this.board.getEntry({x: i, y: j});
    ...
    movePlayerPos(player : Player, u : number, v : number) {
        if (!player) {
            console.error("Player is not initialized!");
            return; // Prevent the error if player is undefined
        }
        this.board.playerLoc = {x: u, y: v};
        const [x, y] = [u * this.tileWidth + this.tileWidth * 3 / 4, v * this.tileHeight + this.tileHeight / 4];
        player.move(x, y);
        console.log("player position: " + u + " " + v);
    }

    //moves player with suggestion of dir, handles collision and time progression
    movePlayerDir(player : Player, dir : number[]) {
        const curLoc = this.board.playerLoc;
        const newX = curLoc.x + dir[0];
        const newY = curLoc.y + dir[1];
        ...
        for (let i = 0; i < this.board.width; i++) {
            for (let j = 0; j < this.board.height; j++) {
                ...
                if(curEntry.crop != undefined){
                    if(this.plantTypes[curEntry.crop].canGrow(curEntry.sunlight, curEntry.moisture, curEntry.growth) ){
                        curEntry.moisture = curEntry.moisture - this.plantTypes[curEntry.crop].moistureConsumption;
                        curEntry.growth += 1;
                        this.cropSprites.get([i,j].toString())!.setStage(curEntry.growth);
                    }
                }
            }
        }
    ...
    textUpdate(x : number, y : number) {
        this.eventEmitter.emit("updateCell" + x + y);
    }

    hudUpdate(){
        this.eventEmitter.emit("hudUpdate");
    }

    plantCrop(mx : number, my : number){
        const [u, v] = [Math.floor(mx / this.tileWidth), Math.floor(my / this.tileHeight)];
        const curLoc = this.board.playerLoc
        const entry = this.board.getEntry({x: u, y: v});
    ...
    addCropSprite(u : number, v : number) {
        const newCrop : Crop = new Crop(this, u * this.tileWidth + this.tileWidth / 2, v * this.tileHeight + this.tileHeight * 3 / 4, this.plantTypes[1].growthFrames, 0);
        const plantScale = (this.tileWidth) / (newCrop.width * 2);
        newCrop.setScale(plantScale, plantScale);
        newCrop.setVisible(false);
        this.cropSprites.set([u,v].toString(), newCrop);
    }

    plantCropSprite(u : number, v : number, type : number, growth : number) {
        console.log("planted");
        const cropSprite = this.cropSprites.get([u,v].toString())!;
        cropSprite.overrideType(this.plantTypes[type].growthFrames, growth);
        const plantScale = (this.tileWidth) / (cropSprite.width * 2);
        cropSprite.setScale(plantScale, plantScale);
        cropSprite.setVisible(true);
    }
    
    harvestCrop(u : number, v : number) {
        const entry = this.board.getEntry({x: u, y: v});
        //let curEntry = this.board.getEntry(i, j);
        if(entry.growth == this.plantTypes[entry.crop!].getLastStage()) {
            this.cropSprites.get([u,v].toString())!.setVisible(false);
    ...
    refreshCropSprite(u : number, v : number) {
        const curEntry = this.board.getEntry({x: u, y: v});
        const curSprite = this.cropSprites.get([u,v].toString())!;
        const entryCrop = curEntry.crop;
    ...
    savePrompt() {
        ...
        const slot : string | null = prompt("Which file would you like to save to? (1-6):", "1");
        ...
        if(match != null) {
            ...
            const choice = confirm("Invalid Slot, would you like to try again?");
            if(choice) {
                this.savePrompt();
            }
        }
    ...
    loadPrompt() {
        ...
        const slot : string | null = prompt("Which file would you like to load from? (1-6):", "1");
        //I call upon thee, dark magic of the regex
        if(slot == null) {
            return;
        }
    ...
    deletePrompt() {
        ...
        const slot = prompt("Which file would you like to delete? (1-6):", "1");
        //I call upon thee, dark magic of the regex
        if(slot == null) {
            return;
        }
        ...
        const match = promptRe.exec(slot);
        if(match != null) {
            const choice = confirm("Are you sure you want to remove slot " + match[1] + "?")
            if(choice) {
