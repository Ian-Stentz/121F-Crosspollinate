...
In order to represent our plants, we use a class instance for each different plant type in order to track growth and crossbreeding requirements. Each plant class is initialized with only placeholder values within a DSL compiler function. the function then creates an specially designed DSL object containing functions that act as setters for the plant type's various fields. Each plant type definition is itself a function that is passed the DSL within the compiler, and then calls its setter functions with specific values. In order to recieve the plant types at the beginning of the main scene, the compiler is passed each of the plant library functions sequentially. Once the DSL file was written, and the farm scene was updated to use the new plant class, additional crops could be added entirely through the library in the load scene. 

Each of the plants, in addition to unique sunlight and water requirements for growth, has a unique set of adjacency bonuses and penalties. In addition to the current sunlight and water, the canGrow() function is also passed the adjacent crops. The growth requirements are then lowered or raised accordingly to represent monocropping issues or symbiosis. Additionally, the adjacency system is used to allow for crossbreeding. Non-starter crops are also passed and array of parents. Whenever an empty tile is adjacent to all the parents of another crop, that crop will begin to grow in the tile.
